...
if [ ! -f "$ENV_FILE" ]; then
  umask 077
  POSTGRES_PASSWORD="$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 32 || true)"
  [ -n "$POSTGRES_PASSWORD" ] || POSTGRES_PASSWORD="$(date +%s)-Alfa-DB"
  printf 'POSTGRES_PASSWORD=%s\n' "$POSTGRES_PASSWORD" > "$ENV_FILE"
  chmod 600 "$ENV_FILE"
elif ! grep -qE '^POSTGRES_PASSWORD=.+' "$ENV_FILE"; then
  umask 077
  POSTGRES_PASSWORD="$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 32 || true)"
  [ -n "$POSTGRES_PASSWORD" ] || POSTGRES_PASSWORD="$(date +%s)-Alfa-DB"
  printf '\nPOSTGRES_PASSWORD=%s\n' "$POSTGRES_PASSWORD" >> "$ENV_FILE"
